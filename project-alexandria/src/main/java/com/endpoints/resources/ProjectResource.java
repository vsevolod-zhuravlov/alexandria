...
public class ProjectResource {
    ...
    @POST
    @Path("/get-all-submits")
    public Response getAllSubmits(
                           @QueryParam("projectAddress") String projectAddress,
                           @QueryParam("taskId") String taskId,
                           @HeaderParam("Authorization") String authHeader) {
        if (authHeader == null || !authHeader.startsWith("Bearer:")) {
        ...
        try {
            ...
            if (hasStudentRights(project, authHeader)) {
                Task task = projectService.getTask(taskId);
                if (task != null) {
                    List<Submit> submits = projectService.getSubmits(task.getSubmits());
                    return Response.ok(submits).build();
                }
                return Response.status(Response.Status.NOT_FOUND).build();
            } else {
                return Response.status(Response.Status.UNAUTHORIZED).build();
    ...
    @POST
    @Path("/get-all-unconfirmed")
    public Response getAllUnconfirmed(
            @QueryParam("projectAddress") String projectAddress,
            @QueryParam("taskId") String taskId,
            @HeaderParam("Authorization") String authHeader) {
        if (authHeader == null || !authHeader.startsWith("Bearer:")) {
        ...
        try {
            ...
            if (hasConfirmerRights(project, authHeader)) {
                ...
                if (task != null) {
                    List<Submit> submits = projectService.getSubmits(task.getSubmits());
                    List<Submit> result = submits.stream()
                            .filter(submit -> submit.isConfirmedBy(walletAddress))
                    .toList();
